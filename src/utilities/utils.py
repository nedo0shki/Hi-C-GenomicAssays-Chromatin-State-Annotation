...
def construct_interchrom_COO(dump_dir, resolution):
    ...
    valid_files = [filename for filename in os.listdir(
        dump_dir) if filename.startswith("input") & filename.endswith(".txt")]
    for file in valid_files:
        ...
        with open(file_path) as f:
            for line in f:
                ...
                first_node_num = node_num(
                    first_chr_num, int(first_chr_pos), resolution)
                second_node_num = node_num(
                    second_chr_num, int(second_chr_pos), resolution)
                o.write(str(first_node_num) + " " +
                        str(second_node_num) + " " + value + "\n")
    o.close()
    ...
    for chrm1 in range(1, 23, 2):
        for chrm2 in range(2, 23, 2):
            output_path = os.path.join(
                tmp_dir, '{2}_chrm{0}_chrm{1}.txt'.format(chrm1, chrm2, prefix))

            if os.path.isfile(output_path) and not overwrite:
                continue

            cmd = 'java -jar {0} dump observed KR {1} {2} {3} BP 100000 {4} > tmp_juicer_log'.format(
                juicer_path, h, chrm1, chrm2, output_path)
            call([cmd], shell=True)
    ...
    for line in original_wig:
        if not line.startswith("#"):
            ...
            if chr_name != last_chr_name:
                ...
                average = 0
                average_coverage = 0
                virtual_wig.write("fixedStep chrom=" +
                                  chr_name + " start=1 step=1 span=1" + "\n")
            coverage = end - start
...
def SNIPER_output_to_wigfix(data_path, index_path, wigfix_dir_path, resolution):
    ...
    for l in range(num_LV):
        LV.append(open(os.path.join(wigfix_dir_path,
                                    "LV" + str(l+1) + ".wigfix"), "w"))
    i = 0
    while i < index_mat['rowMap'].shape[0]:
        header = "fixedStep chrom=chr" + str(index_mat['rowMap'][i, 1]) + " start=" + str(
            (resolution*index_mat['rowMap'][i, 2])+1) + " step=" + str(resolution) + " span=" + str(resolution)
        for l in range(num_LV):
            LV[l].write(header)
            LV[l].write("\n")
    ...
    while i < index_mat['colMap'].shape[0]:
        header = "fixedStep chrom=chr" + str(index_mat['colMap'][i, 1]) + " start=" + str(
            (resolution*index_mat['colMap'][i, 2])+1) + " step=" + str(resolution) + " span=" + str(resolution)
        for l in range(num_LV):
            LV[l].write(header)
            LV[l].write("\n")
...
def hicToIntraMat(hic_path, dump_dir, resolution, juicer_path):
    ...
    for chr_num in range(1, 23):
        ...
        if not os.path.isfile(output_path):
            cmd = 'java -jar {0} dump observed KR {1} {2} {2} BP {3} {4} > tmp_juicer_log'.format(
                juicer_path, hic_path, chr_num, resolution, output_path)
            call([cmd], shell=True)
...
def hicToCorrIntraMat(hic_path, dump_dir, resolution, juicer_path):
    ...
    for chr_num in range(1, 23):
        output_path = os.path.join(
            dump_dir, 'corr_chrom{0}.txt'.format(chr_num))
        if not os.path.isfile(output_path):
            coo_path = os.path.join(dump_dir, 'chrom{0}.txt'.format(chr_num))
            cmd = 'java -jar {0} dump observed KR {1} {2} {2} BP {3} {4} > tmp_juicer_log'.format(
                juicer_path, hic_path, chr_num, resolution, coo_path)
            call([cmd], shell=True)
            coo = pd.read_csv(coo_path, header=None, sep="\t")
            coo.columns = ['source', 'target', 'weight']
            coo[['source', 'target']] = coo[['source', 'target']] / resolution
            expected_contacts = coo.groupby(
                abs(coo['target']-coo['source']), as_index=True).sum()
            chr_vir_size = int(chrom_sizes[chr_num-1]/resolution)
            expected_contacts['weight'] = expected_contacts['weight'] / \
                (chr_vir_size-expected_contacts.index.to_series())
            coo['weight'] = coo['weight'] / \
                expected_contacts.loc[abs(
                    coo['target']-coo['source']), 'weight'].values
            OtoE_path = os.path.join(
                dump_dir, 'chrom{0}_OtoE.txt'.format(chr_num))
            coo.to_csv(OtoE_path, header=None, index=None, sep=' ', mode='a')
            coo_matrix = np.zeros(
                shape=(chr_vir_size, chr_vir_size), dtype=float)
            weight_list = coo['weight'].ravel()
            weight_list[np.isnan(weight_list)] = 0
            coo_matrix[coo['source'].ravel().astype(
                int)-1, coo['target'].ravel().astype(int)-1] = weight_list
            coo_matrix = coo_matrix + coo_matrix.T - \
                np.diag(np.diag(coo_matrix))
            corr_matrix = np.corrcoef(coo_matrix)
            corr_matrix = pd.DataFrame(corr_matrix)
            corr_coo = corr_matrix.rename_axis('source').reset_index().melt(
                'source', value_name='weight', var_name='target').dropna()
            corr_coo.to_csv(output_path, header=None,
